...
test('production HTTP routes, server metadata, redirects, forms, icons and protected areas',async()=>{
 ...
 const sitemap=await(await fetch(base+'/sitemap.xml')).text();assert.ok(sitemap.includes('/visit-us'));assert.ok(!sitemap.includes('/admin'));assert.ok((await(await fetch(base+'/robots.txt')).text()).includes('Disallow: /admin'));
});
