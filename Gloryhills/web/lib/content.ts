...
export async function settings():Promise<Record<string,string>>{const rows=await published('settings');return {...confirmed,...rows.find(row=>row.slug!=='marketing')?.data};}
