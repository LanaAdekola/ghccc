...
create table public.user_roles(user_id uuid primary key references auth.users(id) on delete cascade, role public.admin_role not null default 'admin');
alter table public.user_roles enable row level security;
create function public.has_role(required public.admin_role default 'admin') returns boolean language sql stable security definer set search_path = '' as $$
 select exists(select 1 from public.user_roles where user_id=auth.uid() and (role=required or role='super_admin'));
$$;
...
create table public.content(
 ...
 unique(kind,slug),check(status='draft' or published_at is not null),check(image_url is null or length(image_alt)>0)
);
...
create policy editor_read on public.content for select to authenticated using(public.has_role());
create policy editor_insert on public.content for insert to authenticated with check(public.has_role());
create policy editor_update on public.content for update to authenticated using(public.has_role()) with check(public.has_role());
create policy editor_delete on public.content for delete to authenticated using(public.has_role());
...
create policy submissions_admin_read on public.submissions for select to authenticated using(public.has_role());
create policy submissions_admin_delete on public.submissions for delete to authenticated using(public.has_role());
-- No anon insert: validated server endpoint with CAPTCHA and durable rate limit owns writes.
...
create table public.audit_logs(id bigint generated always as identity primary key,actor uuid,table_name text not null,record_id text,action text not null,created_at timestamptz not null default now());
...
create function public.audit_change() returns trigger language plpgsql security definer set search_path = '' as $$
 begin
 insert into public.audit_logs(actor,table_name,record_id,action) values(auth.uid(),TG_TABLE_NAME,coalesce(to_jsonb(NEW)->>'id',to_jsonb(OLD)->>'id',to_jsonb(NEW)->>'user_id',to_jsonb(OLD)->>'user_id'),TG_OP);
 if TG_OP='DELETE' then return OLD; end if;
 if TG_TABLE_NAME='content' then NEW.updated_at=now(); end if;
 return NEW;
 end;
$$;
...
-- Private originals. Public reads require attachment to a published record.
...
create policy media_admin on storage.objects for all to authenticated using(bucket_id='church-media' and public.has_role()) with check(bucket_id='church-media' and public.has_role());
-- Explicit grants avoid relying on dashboard default privileges; RLS still applies.
